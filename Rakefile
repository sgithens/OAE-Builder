...
task :update do
  for p in projects do
    g = Git.open(p)
    puts g.pull("sakaiproject", "master")
  end
end
...
task :rebuild do
  ...
  for p in projects do
    system("cd #{p} && mvn clean install -Dmaven.test.skip")
  end
end
...
task :setfsresource => [:setuprequests] do
  # upload fsresrouce jar
  abspath = `pwd`.chomp
  c = Curl::Easy.new("http://localhost:8080/system/console/bundles/upload")
  c.http_auth_types = :basic
  c.username = 'admin'
  c.password = 'admin'
  c.multipart_form_post = true
  c.http_post(
    Curl::PostField.file("bundlefile", abspath + "/org.apache.sling.fsresource-1.0.1-SNAPSHOT.jar"),
    Curl::PostField.content("action", "install"),
    Curl::PostField.content("bundlestart", "start"),
